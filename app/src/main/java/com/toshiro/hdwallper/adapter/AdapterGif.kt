...
class AdapterGif(private val context: Context, var arrayList: List<GIFItem>, private val recyclerViewClickListener: RecyclerViewClickListener,private val view : View) : RecyclerView.Adapter<GifViewHoler>() {

    private val database = Database(context)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): GifViewHoler {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.layout_gif, parent, false)
        return GifViewHoler(itemView)
    }

    override fun onBindViewHolder(holder: GifViewHoler, position: Int) {
...
class GifViewHoler (view: View) : RecyclerView.ViewHolder(view) {
