...
class AdapterFavouriteImage(private val context: Context, var arrayList: MutableList<HDWALLPAPERItem>, private val recyclerViewClickListener: RecyclerViewClickListener, private val view: View) : RecyclerView.Adapter<ImageFavouriteViewHoler>() {

    val database = Database(context)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ImageFavouriteViewHoler {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.layout_image_latest, parent, false)
        return ImageFavouriteViewHoler(itemView)
    }

    override fun onBindViewHolder(holder: ImageFavouriteViewHoler, position: Int) {
...
class ImageFavouriteViewHoler (view: View) : RecyclerView.ViewHolder(view) {
